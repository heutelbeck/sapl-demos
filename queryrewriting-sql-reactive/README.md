...
You need Java 21 or later and Maven installed on your system. The demo uses an embedded H2 database, so no separate database installation is required to run the application.

To run the integration tests (`mvn verify -Pit`), you need Docker installed for the Testcontainers-based PostgreSQL tests.
...
```bash
cd queryrewriting-sql-reactive
mvn spring-boot:run
```
...
For custom configuration, you can set these properties in `application.properties`:
