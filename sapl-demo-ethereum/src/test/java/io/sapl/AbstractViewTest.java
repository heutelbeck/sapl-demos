 ...
 * <p>
 * The tests use Chrome driver (see pom.xml for integration-tests profile) to run
 * integration tests on a headless Chrome. If a property {@code test.use
 * .hub} is set to true, {@code AbstractViewTest} will assume that the TestBench test is
 * running in a CI environment. In order to keep the class light, it makes certain
 * assumptions about the CI environment (such as available environment variables). It is
 * not advisable to use this class as a base class for you own TestBench tests.
 * <p>
 * To learn more about TestBench, visit
 * <a href="https://vaadin.com/docs/v10/testbench/testbench-overview.html">Vaadin
 * TestBench</a>.
 */
public abstract class AbstractViewTest extends ParallelTest {
	...
	/**
	 * Convenience method for getting the root element of the view based on the selector
	 ...
	 * {@code themeClass}. If the theme is not found, JUnit assert will fail the test
	 * case.
	...
	protected void assertThemePresentOnElement(WebElement element, Class<? extends AbstractTheme> themeClass) {
		...
		Boolean hasStyle = (Boolean) executeScript("" + "var styles = Array.from(arguments[0]._template.content"
				+ ".querySelectorAll('style'))" + ".filter(style => style.textContent.indexOf('" + themeName
				+ "') > -1);" + "return styles.length > 0;", element);
	...
	/**
	 * Returns whether we are using a test hub. This means that the starter is running
	 * tests in Vaadin's CI environment, and uses TestBench to connect to the testing hub.
