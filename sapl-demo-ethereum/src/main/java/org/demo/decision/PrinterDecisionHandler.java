...
public class PrinterDecisionHandler {
	...
	public Flux<Decision> crowdAccessDecision(PrinterUser user) {
		...
		final Flux<Decision> decisionFlux = crowdAccess.map(AuthorizationDecision::getDecision);
		return decisionFlux;
	}
	...
	public Flux<Decision> paidAccessDecision(PrinterUser user) {
		...
		final Flux<Decision> decisionFlux = paidAccess.map(AuthorizationDecision::getDecision);
		return decisionFlux;
	}
	...
	public Flux<Decision> printerAccessDecision(PrinterUser user, String printer) {
		...
		final Flux<Decision> decisionFlux = printerAccess.map(AuthorizationDecision::getDecision);
		return decisionFlux;
	}
