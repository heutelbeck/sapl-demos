...
public interface PersonRepository extends R2dbcRepository<Person, Integer> {
	 ...
	 * the provided value. It enforces the policy with 'excludeInactivePersons'
	 * action using the {@link QueryEnforce} annotation to exclude inactive persons.
	 *
	 ...
	 * 'excludeInactivePersonsAndSelectFirstname' action using the
	 * {@link QueryEnforce} annotation for users with the 'ROLE_ADMIN' role in their
	 * spring security authentication. The query manipulates the SQL query and
	 * excludes not active persons.
	 *
