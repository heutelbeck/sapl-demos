...
And the policy obligation specifies the condition "category IN (1,2)", SAPL rewrites the query to:

```sql
SELECT * FROM book WHERE category IN (1,2)
```

This filtering happens at the database level, ensuring that unauthorized rows are never transferred to the application.
...
2. Staff members with assigned sections should be permitted to view the catalog, but should only see books from their assigned sections. The policy must generate the appropriate SQL conditions to filter the query results.
...
The second policy permits access for all other cases and attaches an obligation. The obligation contains a list of category IDs that the user is permitted to see. The R2DBC constraint handler interprets this obligation and generates the appropriate SQL WHERE clause.
...
Unlike the books demo which uses @PreEnforce with a custom constraint handler, @QueryEnforce enables SAPL's built-in query manipulation. The framework handles the obligation processing automatically.

### How the Authorization Subscription is Built

When the findAll method is called, SAPL extracts the current authentication from Spring Security and uses it as the subject. The action is derived from the method name and signature. The resource defaults to the repository's entity type.

You can customize these values using the annotation parameters:

```java
@QueryEnforce(subject = "#authentication", action = "'customAction'", resource = "'books'")
Flux<Book> findAll();
```
...
These scenarios verify that each staff member receives an obligation containing exactly their assigned sections. The head librarian receives all five, the children's specialist receives sections 1 and 2, and the classics curator receives only section 5.
...
These tests ensure that delete and save operations are not affected by this policy set, leaving them to be handled by other policies.
...
For the same security scenario using JPA with a custom constraint handler that manipulates method arguments, see the sapl-demo-books project. For the MongoDB equivalent using SAPL's native MongoDB query manipulation, see sapl-demo-spring-data-mongo-reactive.
