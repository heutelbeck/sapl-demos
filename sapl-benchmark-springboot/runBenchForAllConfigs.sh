...
#	echo "IMPROVED - general policies"
#	time java -Xms4096m -Xmx14000m -jar target/sapl-benchmark-springboot-1.0.0-SNAPSHOT.jar \
#		-index $INDEX -iter $ITER --sapl.random.seed=$SEED --sapl.number.of.benchmark.runs=$RUNS
	...
	time java -Xms4096m -Xmx14000m -Dspring.profiles.active=5k -jar target/sapl-benchmark-springboot-1.0.0-SNAPSHOT.jar \
		-index $INDEX -iter $ITER --sapl.random.seed=27338 --sapl.number.of.benchmark.runs=$RUNS
...
#		-index $INDEX -iter $ITER --sapl.random.seed=8311 --sapl.number.of.benchmark.runs=$RUNS
#
...
#		-index $INDEX -iter $ITER --sapl.random.seed=-1479923292 --sapl.number.of.benchmark.runs=$RUNS
#
...
#		-index $INDEX -iter $ITER --sapl.random.seed=-879950416 --sapl.number.of.benchmark.runs=$RUNS
#
...
#		-index $INDEX -iter $ITER --sapl.random.seed=7617 --sapl.number.of.benchmark.runs=$RUNS
