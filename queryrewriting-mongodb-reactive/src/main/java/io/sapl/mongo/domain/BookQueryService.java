 ...
 * point of {@link ReactiveMongoTemplate}, not just the repository path. Every
 * method is enforced by the same {@code findAll} policy, whose obligation
 * restricts {@code category} to the caller's data scope. The shim applies that
 * obligation regardless of which template API the method uses (legacy find, the
 * fluent find chain, or the fluent update / remove builders), so a caller only
 * ever reads or writes the books they are allowed to see.
 */
@Service
...
public class BookQueryService {
    ...
    public Flux<Book> legacyFindAll() {
    ...
    @PreEnforce(action = "'findAll'")
    public Flux<Book> fluentNamedLike(String pattern) {
