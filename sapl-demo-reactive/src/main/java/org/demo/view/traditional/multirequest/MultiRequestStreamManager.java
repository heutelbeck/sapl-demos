 ...
 * multi-requests. It keeps the subscriptions and updates the decisions for the single
 * requests contained in the multi-requests in the session until {@link #dispose()} is
 * called.
 */
@Slf4j
public class MultiRequestStreamManager {
	...
	/**
	 * Creates a new {@code MultiRequestStreamManager} instance delegating to the given
	...
	public void setupNewMultiRequest(String multiRequestId, MultiRequest multiRequest) {
		if (!hasMultiRequestSubscriptionFor(multiRequestId)) {
			...
			if (multiResponse != null) {
				multiResponse
						.forEach(ir -> decisionsByRequestId.put(ir.getRequestId(), ir.getResponse().getDecision()));
				multiRequestIdsWithSubscription.add(multiRequestId);
			}
			final Disposable subscription = pep.filterEnforce(multiRequest)
					.subscribe(ir -> decisionsByRequestId.put(ir.getRequestId(), ir.getResponse().getDecision()));
	...
	/**
	 * Returns {@code true} if the current authorization decision for the request with the
	 * given ID is PERMIT, {@code false} otherwise.
	 * @param requestId the ID of the single request.
	 * @return {@code true} if the current authorization decision for the request with the
	 * given ID is PERMIT.
	 */
