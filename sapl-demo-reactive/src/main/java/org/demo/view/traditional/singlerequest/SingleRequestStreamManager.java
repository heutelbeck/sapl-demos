...
public class SingleRequestStreamManager {
    ...
    /**
     * Creates a new {@code SingleRequestStreamManager} instance delegating
     ...
     * Returns {@code true} if the current authorization decision for the request
     * containing the current Spring authentication as its subject and then given
     * action, resource and environment is PERMIT, {@code false} otherwise. If a
     * subscription to the stream returned by the PEP for this request does not yet
    ...
    public boolean isAccessPermitted(Object action, Object resource, Object environment) {
        ...
        if (keysWithSubscription.add(key)) {
            LOGGER.debug("setup subscription for action {}, resource {} and environment {}", action, resource, environment);
            final Authentication subject = SecurityUtils.getAuthentication();
            final Decision initialDecision = pep.enforce(subject, action, resource, environment)
                    .blockFirst();
