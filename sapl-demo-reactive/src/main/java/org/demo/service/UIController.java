...
/**
 * This controller demonstrates the usage of the {@link PreEnforce @PreEnforce} annotation.
 */
@Component
...
public class UIController {
    ...
    public void updatePatient(Patient patient, AbstractPatientForm form) {
        ...
        if (form.hasDiagnosisTextBeenModified()) {
            patientRepo.updateDiagnosisTextById(patient.getDiagnosisText(), patient.getId());
        }
        if (form.hasAttendingDoctorBeenModified()) {
            patientRepo.updateAttendingDoctorById(patient.getAttendingDoctor(), patient.getId());
        }
        if (form.hasAttendingNurseBeenModified()) {
            patientRepo.updateAttendingNurseById(patient.getAttendingNurse(), patient.getId());
        }
