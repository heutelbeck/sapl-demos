 ...
 * needs to extend the {@link BaseTestAdapter} with the specific Type that represents your target Representation in this Case {@link TestCase} to just have a runnable test without a framework setup.
 */
public class TestAdapter extends BaseTestAdapter<List<TestCase>> {

    /**
     * use the special constructor provided by {@link BaseTestAdapter} to customize policy loading behaviour
     * @param customUnitTestPolicyResolver the custom resolver used for unit tests
     * @param customIntegrationTestPolicyResolver the custom resolver used for integration tests
     */
    public TestAdapter(final UnitTestPolicyResolver customUnitTestPolicyResolver, final IntegrationTestPolicyResolver customIntegrationTestPolicyResolver) {
     ...
     * @param testNodes collection of test nodes being either {@link TestCase} or {@link TestContainer} instances.
     ...
     * @return a Map containing the identifier and {@link Throwable} of each failed tests
     */
    ...
    /**
     * override that is called by the {@link BaseTestAdapter} to convert TestContainer into a list of {@link TestCase} instances.
     * @param testContainer the TestContainer instance
     * @param shouldSetTestSourceUri not used for plain setup since there is no testSourceUri
     * @return list of the converted {@link TestCase} instances
     */
    @Override
    protected List<TestCase> convertTestContainerToTargetRepresentation(TestContainer testContainer, boolean shouldSetTestSourceUri) {
